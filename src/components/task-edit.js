...
export const getEditTaskCardLayout = ({description, dueDate, repeatingDays, tags, color, isFavorite, isArchive}) => {
    ...
    <form class="card__form" method="get">
      <div class="card__inner">
        <div class="card__control">
          <button 
            type="button" class="card__btn card__btn--archive ${isArchive ? `card__btn--disabled` : ``}"
          >
            archive
          </button>
          <button
        ...
        <div class="card__settings">
          <div class="card__details">
            <div class="card__dates">
              ...
              <button class="card__repeat-toggle" type="button">
                crepeat:<span class="card__repeat-status">${Object.keys(repeatingDays).some((day) => repeatingDays[day]) ? `yes` : `no`}</span>
              </button>
          ...
          <div class="card__colors-inner">
            ...
            <div class="card__colors-wrap">
              ${colors.map((it) => `<input
                type="radio"
                id="color-${it}-4" class="card__color-input card__color-input--${it} visually-hidden"
                name="color"
                value="${it}" 
                ${it === color ? `checked` : ``}
              />
              <label
                for="color-${it}-4"
                class="card__color card__color--${it}">${it}</label>`).join(``)}
            </div>
          </div>
        </div>
